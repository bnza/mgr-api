...
class ApiResourceExportHistoryLocationTest extends ApiTestCase
{
    ...
    public function testExportFeatureCollection(string $outputFormat, string $expectedContentType): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', "/api/features/export/history/locations?outputFormat=$outputFormat", [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $this->assertResponseHeaderSame('content-type', $expectedContentType);
        $this->assertResponseHeaderSame('content-disposition', 'attachment; filename="history_locations.'.self::getFileExtension($outputFormat).'"');
    }
    ...
    public function testExportFeatureCollectionFiltered(string $outputFormat, string $expectedContentType): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', "/api/features/export/history/locations?search=castillo&outputFormat=$outputFormat", [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $this->assertResponseHeaderSame('content-type', $expectedContentType);
    ...
    public function testExportFeatureCollectionDefaultFormat(): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', '/api/features/export/history/locations', [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $this->assertResponseHeaderSame('content-type', 'application/geo+json');
        $this->assertResponseHeaderSame('content-disposition', 'attachment; filename="history_locations.geojson"');
    }
    ...
    public function testExportFeatureCollectionRequiresAuthentication(): void
    {
        ...
        $this->apiRequest($client, 'GET', '/api/features/export/history/locations');
    ...
    public function testExportFeatureCollectionInvalidFormat(): void
    {
        ...
        $this->apiRequest($client, 'GET', '/api/features/export/history/locations?outputFormat=invalid', [
            'token' => $token,
        ]);
    ...
    public function testExportFeatureCollectionItemsMatchApiTotalItems(): void
    {
        ...
        $apiResponse = $this->apiRequest($client, 'GET', '/api/data/vocabulary/history/locations?value=vera', [
            'token' => $token,
        ]);
        ...
        $exportResponse = $this->apiRequest($client, 'GET', '/api/features/export/history/locations?value=vera&outputFormat=geojson', [
            'token' => $token,
        ]);
        $this->assertResponseIsSuccessful();
