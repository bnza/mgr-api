...
class ApiResourceExportSamplingSiteTest extends ApiTestCase
{
    ...
    public function testExportFeatureCollection(string $outputFormat, string $expectedContentType): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', "/api/features/export/sampling_sites?outputFormat=$outputFormat", [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $this->assertResponseHeaderSame('content-type', $expectedContentType);
        $this->assertResponseHeaderSame('content-disposition', 'attachment; filename="sampling_sites.'.self::getFileExtension($outputFormat).'"');
    }
    ...
    public function testExportFeatureCollectionFiltered(string $outputFormat, string $expectedContentType): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', "/api/features/export/sampling_sites?name=sediment&outputFormat=$outputFormat", [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $this->assertResponseHeaderSame('content-type', $expectedContentType);
    ...
    public function testExportFeatureCollectionDefaultFormat(): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', '/api/features/export/sampling_sites', [
            'token' => $token,
        ]);
        $this->assertResponseStatusCodeSame(200);
        $this->assertResponseHeaderSame('content-type', 'application/geo+json');
        $this->assertResponseHeaderSame('content-disposition', 'attachment; filename="sampling_sites.geojson"');
    }
    ...
    public function testExportFeatureCollectionRequiresAuthentication(): void
    {
        ...
        $this->apiRequest($client, 'GET', '/api/features/export/sampling_sites');
    ...
    public function testExportFeatureCollectionInvalidFormat(): void
    {
        ...
        $this->apiRequest($client, 'GET', '/api/features/export/sampling_sites?outputFormat=invalid', [
            'token' => $token,
        ]);
    ...
    public function testExportFeatureCollectionItemsMatchApiTotalItems(): void
    {
        ...
        $apiResponse = $this->apiRequest($client, 'GET', '/api/data/sampling_sites?region.value=andalusia', [
            'token' => $token,
        ]);
        ...
        $exportResponse = $this->apiRequest($client, 'GET', '/api/features/export/sampling_sites?region.value=andalusia&outputFormat=geojson', [
            'token' => $token,
        ]);
        $this->assertResponseIsSuccessful();
