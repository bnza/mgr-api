...
class ApiResourcePaleoclimateSamplingSiteTest extends ApiTestCase
{
    ...
    public function testFilterUnaccentedDescriptionGetCollection(): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', '/api/data/paleoclimate_sampling_sites?description=active speleothem');
        $this->assertResponseIsSuccessful();
        $data = $response->toArray();
        $this->assertGreaterThan(0, $data['totalItems']);
        foreach ($data['member'] as $item) {
            $this->assertStringContainsStringIgnoringCase('active speleothem', $item['description']);
        }
    }
    ...
    public function testFilterUnaccentedNameGetCollection(): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', '/api/data/paleoclimate_sampling_sites?name=Cova de sa Galera');
        $this->assertResponseIsSuccessful();
        $data = $response->toArray();
        $this->assertGreaterThan(0, $data['totalItems']);
        foreach ($data['member'] as $item) {
            $this->assertStringContainsStringIgnoringCase('Cova de sa Galera', $item['name']);
        }
    }

    public function testCreateUpdatePaleoclimateSamplingSite(): void
    {
        $client = self::createClient();
        $token = $this->getUserToken($client, 'user_admin');

        $code = 'PCS'.uniqid();
        $name = 'Test Paleoclimate Sampling Site '.uniqid();
        $description = 'A test description with accents: balaghī';

        // 1. Create
        $response = $this->apiRequest($client, 'POST', '/api/data/paleoclimate_sampling_sites', [
            'token' => $token,
            'json' => [
                'code' => $code,
                'name' => $name,
        ...
        // The entity rounds/handles float N/E, we should be able to get them back
    ...
    public function testSearchFilterGetCollection(): void
    {
        ...
        $response = $this->apiRequest($client, 'GET', '/api/data/paleoclimate_sampling_sites?code='.$existingCode);
        $this->assertResponseIsSuccessful();
        $data = $response->toArray();
        $this->assertGreaterThan(0, $data['totalItems']);
        foreach ($data['member'] as $item) {
    ...
    public function testAdminCanDeletePaleoclimateSamplingSite(): void
    {
        $client = self::createClient();
    ...
    public function testPaleoclimateSamplingSiteCodeValidationNotBlank(): void
    {
        $client = self::createClient();
        $token = $this->getUserToken($client, 'user_admin');
    ...
    public function testPaleoclimateSamplingSiteNameValidationNotBlank(): void
    {
        $client = self::createClient();
        $token = $this->getUserToken($client, 'user_admin');
    ...
    public function testPaleoclimateSamplingSiteUniqueCodeConstraint(): void
    {
        $client = self::createClient();
        $token = $this->getUserToken($client, 'user_admin');
    ...
    public function testDeletePaleoclimateSamplingSiteIsBlockedWhenReferencedByOtherEntities(): void
    {
        ...
        // 1. Find a sampling site referenced by a paleoclimate sample
        $response = $this->apiRequest($client, 'GET', '/api/data/paleoclimate_samples', ['token' => $token]);
        $this->assertResponseIsSuccessful();
        $data = $response->toArray();
        $this->assertNotEmpty($data['member']);
        $sample = $data['member'][0];
        $samplingSiteIri = $sample['site']['@id'];

        // 2. Try to delete it
        $this->apiRequest($client, 'DELETE', (string) $samplingSiteIri, ['token' => $token]);
        ...
        $this->assertJsonContains([
            'violations' => [
                [
                    'propertyPath' => '',
                    'message' => 'Cannot delete the sampling site because it is referenced by: PaleoclimateSample.',
                ],
            ],
        ]);
    }

    public function testEditorPaleoclimatologistCanManagePaleoclimateSamplingSites(): void
    {
        $client = self::createClient();

        // Create user with both roles
        $email = 'editor_geo_paleo@example.com';
        $password = 'password123';
        $this->createUser($email, $password, ['ROLE_EDITOR', 'ROLE_PALEOCLIMATOLOGIST']);

        $token = $this->getUserToken($client, $email, $password);

        // 1. GET /api/data/paleoclimate_sampling_sites and check _acl: {canCreate: true}
        $response = $this->apiRequest($client, 'GET', '/api/data/paleoclimate_sampling_sites', ['token' => $token]);
        $this->assertResponseIsSuccessful();
        $data = $response->toArray();
