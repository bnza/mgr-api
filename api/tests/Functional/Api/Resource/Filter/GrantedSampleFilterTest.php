...
class GrantedSampleFilterTest extends ApiTestCase
{
    ...
    public function testGrantedFilterWithAuthenticatedUserWithPrivilegesShouldReturnTheGrantedSamples(): void
    {
        $client = self::createClient();
        ...
        // Get all samples from the sites the user has access to
        $allStratigraphicUnitsResponse = $this->apiRequest($client, 'GET', '/api/data/samples?itemsPerPage=1000', [
            'token' => $this->getUserToken($client, 'user_admin'),
        ]);
        ...
        foreach ($grantedStratigraphicUnits as $su) {
            $this->assertTrue(
                ...
                'Sample should only be from sites where user has privileges'
            );
        }
    }
}
