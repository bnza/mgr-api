...
final class SearchSedimentCoreFilter extends AbstractFilter
{
    ...
    protected function filterProperty(string $property, $value, QueryBuilder $queryBuilder, QueryNameGeneratorInterface $queryNameGenerator, string $resourceClass, ?Operation $operation = null, array $context = []): void
    {
        ...
        if (1 === $chunkCount) {
            $chunk = $chunks[0];
            if (is_numeric($chunk)) {
                // a.2: is numeric -> filter by sediment core number
            ...
            if (!is_numeric($chunk1) && is_numeric($chunk2)) {
                // b.1: c1 is string, c2 is numeric -> site code AND (number OR year)
                $whereConditions[] = $this->createSiteCodeCondition($queryBuilder, $queryNameGenerator, $siteAlias, $chunk1, $parameters);
                $whereConditions[] = $queryBuilder->expr()->orX(
                    $this->createNumberCondition($queryBuilder, $queryNameGenerator, $rootAlias, $chunk2, $parameters),
                    $this->createYearCondition($queryBuilder, $queryNameGenerator, $rootAlias, $chunk2, $parameters)
                );
                ...
                $whereConditions[] = $this->createEmptySetCondition($queryBuilder, $rootAlias);
            }
        } elseif ($chunkCount >= 3) {
    ...
    private function createEmptySetCondition(QueryBuilder $queryBuilder, string $rootAlias): string
    {
        return $queryBuilder->expr()->isNull($rootAlias.'.number');
    }
